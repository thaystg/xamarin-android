...
AndroidSystem::setup_environment_from_override_file (const char *path)
{
	...
	if (name_width > MAX_ENVVAR_NAME_WIDTH) {
		log_warn (LOG_DEFAULT, "Environment override file exceeds the maximum width of variable name (%lu > %lu)", name_width, MAX_ENVVAR_NAME_WIDTH);
		delete[] buf;
		return;
	}
	...
	if (value_width > MAX_ENVVAR_VALUE_WIDTH) {
		log_warn (LOG_DEFAULT, "Environment override file exceeds the maximum width of variable value (%lu > %lu)", value_width, MAX_ENVVAR_VALUE_WIDTH);
		delete[] buf;
		return;
	}
